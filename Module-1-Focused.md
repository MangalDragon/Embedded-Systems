...
**Overleaf / TikZ code (compile as-is) — memory layout:**
...
% Rows:  index / address / LE byte / BE byte
...
**Overleaf / TikZ code (compile as-is) — SPI single-master, two-slave bus:**
...
\begin{tikzpicture}[
  ...
  out/.style={-{Latex[length=2.5mm]}, thick},      % master -> slave
  in/.style={{Latex[length=2.5mm]}-, thick},        % slave  -> master
...
% Master right edge x=1.5 ; Slave left edge x=6.5
% ---- lines to Slave 1 ----
...
**Overleaf / TikZ code (compile as-is) — I²C two-wire bus with pull-ups:**
...
% Each device taps SCL (left pin) and SDA (right pin); crossings have no dot
\foreach \x in {3,6,9}{
...
**Overleaf / TikZ code (compile as-is) — Wi-Fi network:**
...
\begin{tikzpicture}[
  font=\sffamily,
  dev/.style={draw, thick, rounded corners, fill=blue!7,
              minimum width=2cm, minimum height=1cm, align=center},
  ap/.style={draw, very thick, rounded corners, fill=orange!18,
             minimum width=2.8cm, minimum height=1.5cm, align=center},
  cloud/.style={draw, thick, cloud, cloud puffs=12, fill=gray!12,
                minimum width=3.2cm, minimum height=2cm, align=center},
  rf/.style={-{Latex[length=2.5mm]}, thick, dashed}
]
\node[dev]   (A) at (0, 2)  {Device A};
\node[dev]   (B) at (0, 0)  {Device B};
\node[dev]   (C) at (0,-2)  {Device C};
\node[ap]    (R) at (5, 0)  {Wi-Fi Router /\\ Access Point};
\node[cloud] (I) at (10,0)  {Internet};

\draw[rf] (A) -- node[above,font=\scriptsize]{Wi-Fi} (R);
\draw[rf] (B) -- node[above,font=\scriptsize]{Wi-Fi} (R);
\draw[rf] (C) -- node[below,font=\scriptsize]{Wi-Fi} (R);
\draw[-{Latex[length=2.5mm]}, very thick] (R) -- node[above]{IP} (I);
\end{tikzpicture}
\end{document}
```
...
% Caption
\node[font=\itshape] at (0, -4.0)
  {Word 56AB98FC: MSB = 56 (Byte\,3), LSB = FC (Byte\,0)};
\end{tikzpicture}
\end{document}
```
...
**Overleaf / TikZ code (compile as-is) — elements of an embedded system:**
...
\draw[ar] (SEN) -- (C);   % sensors feed the core
\draw[ar] (C)   -- (ACT); % core drives the actuators
\end{tikzpicture}
\end{document}
```
